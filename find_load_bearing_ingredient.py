        ...
        for rid in ids:
            ...
            with model as m:
                ...
                if rid in m.reactions:
                    m.reactions.get_by_id(rid).lower_bound = 0  # zero out just this one contributor
                sol = m.optimize()
                g = sol.objective_value if sol.status == "optimal" else 0.0
            pct = 100 * g / baseline_growth
            flag = "  <-- LOAD-BEARING" if pct < 95 else ""
            if pct < 99.9:  # only print ones that show ANY effect, to keep output readable
                print(f"   {rid:18s} -> growth = {g:.4f}  ({pct:5.1f}% of baseline){flag}")
